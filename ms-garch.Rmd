...
本文所收集的样本区间是2006年10月8日至2018年12月29日，总
计3060个SHIBOR隔夜拆借率数据。数据全部来自于上海银行间同业拆借利率官
网(http://www.shibor.org/)。假设 ${R_t}$ 为第 $t$ 期的隔夜拆借利率，在探讨金融时间
序列变化时，为了消除数据的非平稳性，常常使用对数收益率：

$${r_t} = \ln {R_t} - \ln {R_{t - 1}}$${#eq:ms-model8}

其中， ${R_{t - 1}}$ 为上一期的隔夜拆借利率，通过计算，可以得到3059个上海银行隔夜
...
```{r fig-ts1,eval=T,echo=FALSE,fig.cap = "SHIBOR隔夜拆放利率和对数收益率时序图", dev='png'}
par(mfrow=c(2,1))
par(mar=c(2,4,1,2))
...
由图 \@ref(fig:fig-ts1)  可以看出，SHIBOR隔夜拆借利率在2006-2010年波动幅度较平稳，在2010-2016年
波动幅度较大，在2016-2018年波动趋于平稳。对数收益率序列的方差随时间变化而变化，
...
```{r stat1, eval=T,echo=F,message=FALSE,warning=FALSE,comment=""}
...
为 `r shibor.rt2[1,4]` ，而正态分布的偏度和峰度分别为0和3，故对数收益率序列具有右偏性
和尖峰性，初步判断不服从正态分布。

```{r fig-ts2,eval=T,echo=F,fig.align="center",fig.cap="SHIBOR收益率频率分布直方图和正态Q-Q图", dev='png'}
par(mfrow=c(1,2))
#绘制直方图
...
图 \@ref(fig:fig-ts2)  的直方图和QQ图也显示对数收益率序列不服从正态分布。直方图中，对数收益率序列的尾
...
```{r fig-ts3, eval=T, echo=F,fig.align="center",fig.cap="SHIBOR收益率自相关图和偏自相关图", dev='png'}
#自相关性检验
par(mfrow=c(1,2)) 
...
Ljung-Box Q统计量的值为`r Box1$statistic`，对应p值为`r Box1$p.value`,远小于显著性水平0.05，
故收益率序列为非白噪声序列，具有较强的自相关性，需要进一步提取观察值序列的相关信息。
综合之前的平稳性检验，可以判断对数收益率序列是一个蕴涵相关信息的平稳序列，故建
立ARMA(p,q)线性模型来拟合该序列的变化过程。

```{r fig-ts4, eval=T, echo=F,fig.align="center",fig.cap="SHIBOR收益率残差序列图", dev='png'}
plot(shibor.date[-1],res.ts,type="l",xlab="日期",ylab="残差",cex.main=0.95,las=1)
```
图 \@ref(fig:fig-ts4)  中的自相关系数和偏自相关系数均显示出拖尾的性质，考虑到模型的简洁性，本文将ARMA模型参数p,q
的取值设置为1，2，尝试使用ARMA(1,1)、ARMA(1,2)、
ARMA(2,1)、ARMA(2,2)对序列进行拟合。最终根据参数显著性检验以
及AIC和BIC准则选择最优模型为ARMA(1,2)，其AIC和BIC值最小，
分别为`r fit2$aic`和`r BIC(fit2)`。根据已经得出的参数估计值，
可以获得ARMA(1,2)模型的残差序列，如图4所示。对其进行白噪声检验，
Ljung-Box Q统计量显示残差序列无自相关性，p值为`r Box2$p.value`，小于0.05的显著性水平。
...
```{r stat2, eval=F, echo=F,message=F,warning=F}
#ARMA-GARCH
##拟合基于正态分布的arma(1,2)-GARCH(1,1)模型
m1<-garchFit(~arma(1,2)+garch(1,1),data =shibor.rt ,trace = F)
#summary(m1)
m1.res=c("norm",m1@fit$ics[-c(3,4)])
##拟合基于t分布的arma(1,2)GARCH(1,1)模型
m3<-garchFit(formula = ~arma(1,2)+garch(1,1),data =shibor.rt,trace = F,cond.dist = "std")
#summary(m3)#
m3.res=c("std",m3@fit$ics[-c(3,4)])
##拟合基于ged分布的arma(1,2)GARCH(1,1)模型
m5<-garchFit(formula = ~arma(1,2)+garch(1,1),data =shibor.rt,trace = F,cond.dist = "ged")
#summary(m5)#所估参数的残差分析出现空值，舍去
m5.res=c("ged",m5@fit$ics[-c(3,4)])
##拟合拟合基于snorm分布的arma(1,2)GARCH(1,1)模型
m2<-garchFit(formula = ~arma(1,2)+garch(1,1),data =shibor.rt,trace = F,cond.dist = "snorm")
#summary(m2)#
m2.res=c("snorm",m2@fit$ics[-c(3,4)])
##拟合基于sstd分布的arma(1,2)GARCH(1,1)模型
m4<-garchFit(formula = ~arma(1,2)+garch(1,1),data =shibor.rt,trace = F,cond.dist = "sstd")
#summary(m4)
m4.res=c("sstd",m4@fit$ics[-c(3,4)])
##拟合基于sged分布的arma(1,2)GARCH(1,1)模型
m6<-garchFit(formula = ~arma(1,2)+garch(1,1),data =shibor.rt,trace = F,cond.dist = "sged")
#summary(m6)#所估参数出现空值，舍去
m6.res=c("sged",m6@fit$ics[-c(3,4)])

res=data.frame(m1.res,m2.res,m3.res,m4.res,m5.res,m6.res)
res1=t(res)
colnames(res1)=c("分布形态","AIC","BIC")
rownames(res1)=c("ARMA(1,2)-GARCH(1,1)","ARMA(1,2)-GARCH(1,1)","ARMA(1,2)-GARCH(1,1)","ARMA(1,2)-GARCH(1,1)","ARMA(1,2)-GARCH(1,1)","ARMA(1,2)-GARCH(1,1)")
kable(res1,caption = "ARMA-GARCH模型优度总览")
```
##基于MS-GARCH模型构建
由于高阶GARCH模型会导致ARCH项和GARCH项系数过多，不利于模型稳定和参数估计，而且之前大量学
者的研究表明，低阶GARCH能够刻画时间序列的异方差现象，且在精度方面不存在太大的差距，故设置
ARCH项和GARCH项的阶数均为1,即GARCH(1,1)结构。在分布类型的选取方面，基于3.2.1节的分析可知
，SHIBOR对数收益率序列存在偏态和尖峰厚尾特征，因此分布类型依次选取传统的正态分布、对厚尾
特征刻画更具灵活性的广义t分布和广义误差分布，以及它们的偏态分布，即“norm”、"std"、"ged"、
“snorm”、"sstd"、"sged"共6种。另外设置区制K为1、2、3共3种情况。本文选用R的MSGARCH包作为计
量工具。对平稳不相关但具有异方差性的残差序列共构建215个模型，根据参数显著性检验、极大似然
值以及AIC和BIC准则的方法，最终选择3阶段的GARCH_sstd、GARCH_sstd、GARCH_ged为最优模型，具体
结果详见表\@ref(tab:stat2)。
...
m7=FitML(spec,shibor.rt)
#summary(m7)
m7.res=c("norm-norm",summary(m7)$AIC,summary(m7)$BIC)

####混合分布的构成为norm、snorm
spec=CreateSpec(variance.spec = list(model=c("sGARCH","sGARCH")),distribution.spec = list(distribution=c("norm", "snorm")))
m8=FitML(spec,shibor.rt)
#summary(m8)
m8.res=c("norm-snorm",summary(m8)$AIC,summary(m8)$BIC)
...
#summary(m9)
m9.res=c("norm-std",summary(m9)$AIC,summary(m9)$BIC)
...
#summary(m10)
m10.res=c("norm-sstd",summary(m10)$AIC,summary(m10)$BIC)
...
#summary(m11)
m11.res=c("norm-ged",summary(m11)$AIC,summary(m11)$BIC)
...
#summary(m12)
m12.res=c("norm-sged",summary(m12)$AIC,summary(m12)$BIC)
...
#summary(m13)
m13.res=c("snorm-snorm",summary(m13)$AIC,summary(m13)$BIC)
...
#summary(m14)
m14.res=c("snorm-std",summary(m14)$AIC,summary(m14)$BIC)
...
#summary(m15)
m15.res=c("snorm-sstd",summary(m15)$AIC,summary(m15)$BIC)
...
#summary(m16)
m16.res=c("snorm-ged",summary(m16)$AIC,summary(m16)$BIC)
...
#summary(m17)
m17.res=c("snorm-sged",summary(m17)$AIC,summary(m17)$BIC)
...
#summary(m18)
m18.res=c("std-std",summary(m18)$AIC,summary(m18)$BIC)
...
#summary(m19)
m19.res=c("std-sstd",summary(m19)$AIC,summary(m19)$BIC)
...
#summary(m20)
m20.res=c("std-ged",summary(m20)$AIC,summary(m20)$BIC)
...
#summary(m21)
m21.res=c("std-sged",summary(m21)$AIC,summary(m21)$BIC)
...
#summary(m22)
m22.res=c("sstd-sstd",summary(m22)$AIC,summary(m22)$BIC)
...
#summary(m23)
m23.res=c("sstd-ged",summary(m23)$AIC,summary(m23)$BIC)
...
#summary(m24)
m24.res=c("sstd-sged",summary(m24)$AIC,summary(m24)$BIC)
...
#summary(m25)
m25.res=c("ged-ged",summary(m25)$AIC,summary(m25)$BIC)
...
#summary(m26)
m26.res=c("ged-sged",summary(m26)$AIC,summary(m26)$BIC)
...
#summary(m27)
m27.res=c("sged-sged",summary(m27)$AIC,summary(m27)$BIC)
...
#summary(m28)
m28.res=c("norm、norm、norm",summary(m28)$AIC,summary(m28)$BIC)
...
#summary(m29)
m29.res=c("snorm-snorm-snorm",summary(m29)$AIC,summary(m29)$BIC)
...
#summary(m30)
m30.res=c("std-std-std",summary(m30)$AIC,summary(m30)$BIC)
...
#summary(m31)
m31.res=c("sstd-sstd-sstd",summary(m31)$AIC,summary(m31)$BIC)
...
#summary(m32)
m32.res=c("ged-ged-ged",summary(m32)$AIC,summary(m32)$BIC)
...
#summary(m33)
m33.res=c("sged-sged-sged",summary(m33)$AIC,summary(m33)$BIC)

##四阶段混合
###混合形式为sGARCH、sGARCH、sGARCH、sGARCH
####混合分布的构成为norm、norm、norm、norm#####无结果
...
#summary(m34)
#m34.res=c("norm-norm-norm-norm",summary(m34)$AIC,summary(m34)$BIC)
...
#summary(m35)
m35.res=c("snorm-snorm-snorm-snorm",summary(m35)$AIC,summary(m35)$BIC)
...
#summary(m36)
m36.res=c("std-std-std-std",summary(m36)$AIC,summary(m36)$BIC)
...
#summary(m37)
m37.res=c("sstd-sstd-sstd-sstd",summary(m37)$AIC,summary(m37)$BIC)
...
#summary(m38)
m38.res=c("ged-ged-ged-ged",summary(m38)$AIC,summary(m38)$BIC)
...
#summary(m39)
m39.res=c("sged-sged-sged-sged",summary(m39)$AIC,summary(m39)$BIC)
...
#summary(m40)
m40.res=c("norm-norm",summary(m40)$AIC,summary(m40)$BIC)
...
#summary(m41)
m41.res=c("snorm-snorm",summary(m41)$AIC,summary(m41)$BIC)
...
#summary(m42)
m42.res=c("std-std",summary(m42)$AIC,summary(m42)$BIC)
...
#summary(m43)
m43.res=c("sstd-sstd",summary(m43)$AIC,summary(m43)$BIC)
...
#summary(m44)
m44.res=c("ged-ged",summary(m44)$AIC,summary(m44)$BIC)
...
#summary(m45)
m45.res=c("sged-sged",summary(m45)$AIC,summary(m45)$BIC)
...
#summary(m46)
m46.res=c("norm-norm-norm",summary(m46)$AIC,summary(m46)$BIC)
...
#summary(m47)
m47.res=c("snorm-snorm-snorm",summary(m47)$AIC,summary(m47)$BIC)
...
#summary(m48)
m48.res=c("std-std-std",summary(m48)$AIC,summary(m48)$BIC)
...
#summary(m49)
m49.res=c("sstd-sstd-sstd",summary(m49)$AIC,summary(m49)$BIC)
...
#summary(m50)
m50.res=c("ged-ged-ged",summary(m50)$AIC,summary(m50)$BIC)
...
#summary(m51)
m51.res=c("sged-sged-sged",summary(m51)$AIC,summary(m51)$BIC)
...
#summary(m52)
m52.res=c("norm-norm-norm-norm",summary(m52)$AIC,summary(m52)$BIC)
...
#summary(m53)
m53.res=c("snorm-snorm-snorm-snorm",summary(m53)$AIC,summary(m53)$BIC)
...
#summary(m54)
m54.res=c("std-std-std-std",summary(m54)$AIC,summary(m54)$BIC)
...
#summary(m55)
m55.res=c("sstd-sstd-sstd-sstd",summary(m55)$AIC,summary(m55)$BIC)
...
#summary(m56)
m56.res=c("ged-ged-ged-ged",summary(m56)$AIC,summary(m56)$BIC)
...
#summary(m57)
m57.res=c("sged-sged-sged-sged",summary(m57)$AIC,summary(m57)$BIC)
```

```{r,eval=T, include=F,message=F,warning=F}
res=read.csv(file="E:/res.csv")
kable(res,caption = "MS-GARCH模型优度总览")
```


本文之前的分析指出SHIBOR对数收益率序列有阶段性波动特征，2006-2010年、2010-2016年、
2016-2018年这3个时间段呈现不同的波动幅度。而以上所使用的模型均是在假定某个单一分布
的前提下进行的，在描述对数收益率序列的波动变化过程时，具有片面性和不准确性。故接下
来使用MSGARCH模型进行序列拟合，通过一个不可观测的一阶内生遍历马尔科夫链 ${s_t}$ 和转移矩
阵P，可以实现不同区制的转换，从而对不同波动率进行不同的刻画描述，使得模型精度更高。

```{r stat3,eval=F,echo=F,message=F,warning=F}
res=data.frame(m7.res,m8.res,m9.res,m10.res,m11.res,m12.res,m13.res,m14.res,m15.res,m16.res,m17.res,m18.res,m19.res,m20.res,m21.res,m22.res,m23.res,m24.res,m25.res,m26.res,m27.res,m28.res,m29.res,m30.res,m31.res,m32.res,m33.res,m35.res,m36.res,m37.res,m38.res,m39.res,m40.res,m41.res,m42.res,m43.res,m44.res,m45.res,m46.res,m47.res,m48.res,m49.res,m50.res,m51.res,m52.res,m53.res,m54.res,m55.res,m56.res,m57.res)
res2=t(res)
colnames(res2)=c("分布形态","AIC","BIC")
rownames(res2)=c("MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(2)-GARCH","MS(3)-GARCH","MS(3)-GARCH","MS(3)-GARCH","MS(3)-GARCH","MS(3)-GARCH","MS(3)-GARCH","MS(4)-GARCH","MS(4)-GARCH","MS(4)-GARCH","MS(4)-GARCH","MS(4)-GARCH","MIX(2)-GARCH","MIX(2)-GARCH","MIX(2)-GARCH","MIX(2)-GARCH","MIX(2)-GARCH","MIX(2)-GARCH","MIX(3)-GARCH","MIX(3)-GARCH","MIX(3)-GARCH","MIX(3)-GARCH","MIX(3)-GARCH","MIX(3)-GARCH","MIX(4)-GARCH","MIX(4)-GARCH","MIX(4)-GARCH","MIX(4)-GARCH","MIX(4)-GARCH","MIX(4)-GARCH")
kable(res2,caption = "MS-GARCH模型与混合分布GARCH模型优度总览")
#以上模型根据AIC和BIC准则，选择41、53
```

表\@ref(tab:stat2)中包含k个区制的MS-GARCH模型用“MS(K)-GARCH”表示，包含k种分布的混合分布GARCH模型
用“MIX(k)-GARCH”表示。前6个模型为一般的GARCH模型，假设分布为单一分布，即 $K = 1$ ，显然假
设分布为t分布的模型显著优于正态分布和广义误差分布; $K = 2$ 的两区制马尔科夫转换模型共有36个，其
余为 $K = 3$ 的MSGARCH模型。
...
表给出了最优模型的拟合结果，所有参数均通过显著性检验，极大似然值、AIC和BIC值分别为
6377.884、-12715.768、-12595.251。其中，alpha0_1,alpha0_2,alpha0_3分别表示3个区制
GARCH_sstd、GARCH_sstd、GARCH_ged中的常数项，其值均为0；alpha1_1,alpha1_2,alpha1_3,
分别表示模型的ARCH项系数，其值越大，表明当期收益率对下一期波动率的影响程度越大；beta_1,
beta_2、beta_3分别表示模型的GARCH项系数，该值表示当期波动率对下一期波动率的影响；nu和xi
均应大于0，分别指sged的尾部参数和偏度参数，,xi_1的值为1.5997，大于1，表示第一取值的残差
分布为右偏，xi_2的值为0.8344，小于1，表示第二取值残差分布呈左偏。

分区制对各项系数进行比较可以发现，第一取值中GARCH项系数值为0.6472，大于ARCH项系数0.35，
表明在第一状态中，当期对数收益率序列的波动率对下一期的波动率起决定性作用，这也
说明第一区制的波动率具有聚集效应，具有长期记忆性。第三区制的结果类似且其长期记忆性比第
一取值更显著，GARCH项系数为0.9877。但在第二区制，情况恰好相反，ARCH项系数在下一期的波
动率变化中起更大作用，ARCH项系数为0.9364，远大于GARCH项系数0.0609，说明当期的收益率对
下一期的波动率影响更大。另外，通过计算，可以得到各个取值的无条件方差，即当对数收益率处
于第一取值GARCH-sstd时，无条件波动率为0.1663889，呈低频波动；当对数收益率处于第二取值
GARCH-sstd时，无条件波动率0.4838072，波动幅度有所增加；当对数收益率处于第三取值GARCH-ged时，
其无条件波动率达到最高，为12.3301966，呈高频波动。可见SHIBOR对数收益率的波动率有逐渐增
大的趋势。以上分析综合说明，对数收益率序列存在区制的转换，不同区制呈现出不同特征。

```{r stat5,eval=F,echo=F,message=F,warning=F}
...
表展示了该模型的转移矩阵，其中转移矩阵中的每个元素表示从当前状态转移
到下一状态的概率。以第一行为例，由于最优模型有3个区制，故由当前t时刻的第一状态转移到下
一时刻共有3种情况，3个元素分别对应转移到3个区制的概率，其和为1。p_1_1，值为0.9141，表示
在t时刻所处状态为第一区制的情况下，下一时刻仍为第一区制的概率，其他元素类似。矩阵显示对
角线元素在各行中均为最大值，即两个时刻连续保持同一状态的概率最大，对数收益率的波动更容易
保持一个状态不变。同时，尽管转移到其他状态的概率较小，但状态之间的转移的确存在。若p值为0，
则表示不存在相应的状态转换，如p_3_1的值为0，即表示不存在由第3区制转移到第1区制的情况，但
第1区制转移到第3区制的情况以0.0859的概率发生，表明不同状态之间的转换不具有对称性。

除了转移矩阵，同时还可以得到对数收益率序列的波动分别处于3个状态的无条件概率，结果显示
没有明显的概率差异，其中处于第一状态的概率最大，为0.3425，处于第二状态的概率最小，为0.3176.
说明SHIBOR的对数收益率更倾向于在不同状态之间转化，且在状态的无条件概率几乎相同。
