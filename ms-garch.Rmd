...
金融时间序列的分布较正态分布而言，尾巴拖得更长，中间峰顶更尖，即具有尖峰厚尾的特征。通常我们假设GARCH(p,q)模型的残差服从正态分布，但金融序列的尖峰厚尾现象使得正态分布的假定可能会造成模型设定误差。因此，为了更好的拟合尖峰厚尾现象，通常使用具有尖峰后尾性特征的t分布、广义误差分布（GED）分布来拟合残差的分布。但在很多情况下，t分布和GED分布对于收益率残差序列的特征的拟合不够充分。故本文首先引入Mixing-GARCH混合分布模型和Markov-switching-GARCH马尔科夫转换模型，然后再设置模型的残差分布
对收益率序列进行拟合，下面对MSGARCH模型进行介绍。
##MSGARCH
经典的GARCH（1，1）

${y_t} ={h_t}{u_t} \tag{1}$

$h_t^2 = \omega  + \alpha \varepsilon _{t - 1}^2 + \beta h_{t - 1}^2 \tag{2}$

${\mu _t}$满足独立同分布，其均值为0，方差为1。为了确保条件方差为正，一般限制$\omega  > 0$,$\alpha  > 0$,$\beta  > 0$，当假设$\mu_t>0$为常数，则$\alpha+\beta$测量条件方差的冲击持续性。对于高频数据，$\alpha+\beta$的和往往趋近于1，表示波动过程是高度持久的。

广义t分布

广义$t$分布，即经典$t$分布通过引入一个位置参数( parameter) 和一个规模参数(scale parameter) 后可以转化为一个含有三个参数的位置-规模族(location-scale family)。

在统计学中，位置-规模族是指通过规模参数和位置参数进行参数化后的概率分布族。

设随机变量$T$服从自由度为$v$的经典$t$分布，随机变量$x$服从含有三个参数$(v,\mu ,\sigma )$的广义$t$分布，$x$的密度函数为
${f_G}(x\left| {v,\mu ,\sigma ) = \frac{{\Gamma (\frac{{v + 1}}{2})}}{{\Gamma (\frac{v}{2})\sqrt {\pi v\sigma } }}{{(1 + \frac{1}{v}{{(\frac{{x - \mu }}{\sigma })}^2})}^{ - \frac{{v + 1}}{2}}}} \right.$

$T$与$x$的关系式为：

$T = \frac{{x - \mu }}{\sigma }$

TGARCH模型又称为门限ARCH模型。一般的TGARCH (p, q, k) 模型为：

$\left\{ {\begin{array}{*{20}{c}}
{{x_k} = f(t,{x_{k - 1}},{x_{k - 2}},...) + {\varepsilon _t}}\\
{{\varepsilon _t} = {\sigma _t}{e_t}}\\
{\sigma _t^2 = \omega  + \sum\limits_{j = 1}^q {{\lambda _j}\varepsilon _{t - j}^2 + \sum\limits_{i = 1}^p {{\eta _i}\sigma _{t - i}^2 + \sum\limits_{k = 1}^r {{v_k}\mu _{t - k}^2{I_{t - k}}} } } }
\end{array}} \right.$

上式中,${f(t,{x_{k - 1}},{x_{k - 2}},...)}$为$\left\{ {{x_t}} \right\}$的自回归模型。${{\rm{e}}_t} \sim {\rm{N (0, 1)}}$,当${\varepsilon _t} > 0$时,${I_i} = 1$, 而${\varepsilon _t} < 0$时,${I_i} = 0$,在这种情况下, 可以认为历史的一个正冲击${\varepsilon _t} > 0$对当前波动的影响效应要比历史的一个负冲击${\varepsilon _t} \le 0$ 对当前波动的影响效应的影响要大。如果相反, 即负冲击效应比正冲击效应大, 则当${\varepsilon _t} > 0$时, ${I_i} = 0$;当${\varepsilon _t} \le 0$时,${I_i} = 1$ 。

以上分别是最常用的GARCH(1,1)和TGARCH模型的简单介绍，MSGARCH是GARCH模型的推广，它允许不同状态转化且在每个状态下，参数的设定不同，因此，条件方差具有不同的持久性。
这能很好地拟合波动率聚集的特征。令${y_t}$是时刻$t$的日利率，我们关注动态的波动率，因此假定$E({y_t}) = 0$，并且${y_t}$序列不相关，令$\left\{ {{S_t}} \right\}$是一个遍历齐次马尔可夫链，${s_t}$根据未观测到的一阶遍历齐次马尔可夫链进化，

即转移矩阵$P = \left[ {\begin{array}{*{20}{c}}
{{p_{1,1}}}& \cdots &{{p_{1,K}}}\\
 \vdots & \ddots & \vdots \\
{{p_{K,1}}}& \cdots &{{p_{K,K}}}
\end{array}} \right]$
${p_{i,j}}$表示第t-1期从状态i到第t期状态j的转移概率，即从${s_{t - 1}} = i$到${s_t} = j$的概率。显然${p_{i,j}}$需满足以下两个条件：$0 < {p_{i,j}} < 1,\forall i,j \in \left\{ {1, \cdots ,K} \right\}$,$\sum\limits_{j = 1}^K {{P_{i,j}} = 1,} \forall i \in \left\{ {1, \cdots ,K} \right\}$
...
$D\left( {0,{h_{k,t}},{\xi _k}} \right)$表示条件分布，服从零均值，条件方差${h_{k,t}}$随时间变化，${\xi _k}$是包含$\omega$ ,$\alpha$ ,$\beta$的形状参数向量，例如表现为拖尾且不对称。
${y_t}$的条件方差假定服从GARCH类模型。因此，在${s_t} = k$的条件下，${h_{k,t}}$为第$t$期的方差函数，${h_{k,t - 1}}$为上一期的方差，${\theta _k}$是每个区制独立的条件向量参数，
即：
...
定义GARCH模型条件异方差部分，除了设定模型形式，还有残差分布，常用的条件分布形式有：正态分布、学生t分布和广义误差分布。
...
##模型估计
目前，MSGARCH和Mixture of GARCH模型的估计方法有两种，一种是极大似然估计法，另一种是基于贝叶斯方法的MCMC法。

##二、MS-GARCH模型选择依据
普通GARCH模型只能刻画数据在一种状态时的方差的性质，但对于金融数据而言，外部冲击往往会使数据的波动状况发生改变。比如央行近年频繁采取“降准”的货币政策，这会使银行间短期资金供给增加，同业拆借利率降低，波动性也逐渐平缓。此时如果继续采用普通GARCH模型，会使模型与真实情况相差较大，高估同业拆借市场的风险，不利于对我国利率市场的管控。而使用马尔科夫链与GARCH模型的混合模型则可以刻画同业拆借利率方差的动态变化，即在序列高波动和低波动时期分别拟合不同的GARCH模型。同时，由于经济事件往往具有周期性，过去某一时期的波动性状况在未来也可能会复现，针对该种情况，就需要一个状态转换的MS-GARCH模型来进行描述。
