...
\varepsilon ^{(s_t)} \sim N\left(0,\sigma ^{(s_t)} \right)
$$ {#eq:ms-model}

[@eq:ms-model] 式中 ${y_t:t=1,2,…,T}$ 表示金融时间序列， $k$ 表示区制的个数，$n_1$ , $n_2$
分别表示外生变量和自回归的阶数。设共有 $k$ 个区制， $s_t$ 表示 $y_t$ 所处的
区制的状态。不同区制的转移遵循具有以下转移矩阵的马尔科夫随机过程：

$${P_{ij}} = \Pr \left( {{S_{t + 1}} = i\left| {{(s_t)} = j} \right.} \right)
且\sum\limits_{i = 1}^k {{P_{ij}}}  = 1$$
...
\sum\limits_{j = 1}^q {{\beta _j}{\varepsilon _{t - j}}} }
\end{array}} \right.$$

其中， ${y_t}$ 表示时刻 $t$ 的收益率, $h_t$ 表示条件方差， ${e_t}$ 是均值
为0，方差相等的独立同分布随机变量序列。第一个方程为均值方程，用来描述收益
...
$${y_t} = {\mu _{(s_t)}} + {\varepsilon _{{(s_t)}t}}$$

其中 $\mu _{(s_t)}$ 表示 $y_t$ 在状态 $s_t$ 下的均值方程

$${\mu _{(s_t)}} = {\phi _{{(s_t)}0}} + {\phi _{{(s_t)}1}}{y_{t - 1}} + 
\cdots  + {\phi _{{(s_t)}q}}{y_{t - q}}$$

$\varepsilon _{{(s_t)}t}$ 表示 $y_t$ 在状态 $s_t$ 下的条件方差方程，服从
如下 $GARCH(p,q)$ 模型：

$$\left\{ {\begin{array}{l}
{{\varepsilon _{{(s_t)}t}} = \sqrt {{h_{{(s_t)}t}}} {e_t}} \\
{{h_{{(s_t)}t}} = {\omega _{(s_t)}} + \sum\limits_{i = 1}^p {{\alpha _{{(s_t)}i}}{h_{{(s_t)}t - i}}}  + 
\sum\limits_{j = 1}^q {{\beta _{{(s_t)}j}}{\varepsilon _{{(s_t)}t - j}}} }
\end{array}} \right.$$

其中 ${e_t}$ 是均值为0，方差相等的独立同分布随机变量序列。$\omega
_{(s_t)},\alpha _{(s_t)},\beta _{(s_t)}$ 分别表示在当前区制下的各参数的值。不同
区制的转移由一个k状态的的马尔科夫随机过程控制，状态转移矩阵如下：
...
混合正态分布是由两个或两个以上正态分布的加权组合形成的分布，任意两个
正态分布之间均是独立的。由于不同的正态分布有着不同的均值和方差,因此混
合正态分布通常比正态分布能够更好地刻画尖峰性和厚尾性。而且在本文所使
用的MSGARCH包中可以通过设置参数得到混合正态分布的具体形式。因此，我们
将混合正态分布作为MSGarch的一种特殊情况来分析。

# 实证分析

## 数据来源及数据处理

银行间同业拆放利率是最早市场化的利率，在中国利率体系中扮演着重要的角色。其中上海
银行间同业拆放利率(Shanghai Interbank Offered Rate,SHIBOR)自2006年10月成
立以来，
在近年尤受重视。目前公布的品种依据时间划分主要包括隔夜（O/N）、1周（1W）、2周（2W）
、1个月（1M）等品种。

本文所收集的样本范围是2006年10月8日至2018年12月29日，总计3060个SHIBOR隔夜拆放率数
据。数据全部来自于上海银行间同业拆放利率官网(http://www.shibor.org/) 。假设 
${R_t}$ 为第 $t$  期的隔夜拆放利率，在探讨金融时间序列变化时，为了消除数据的非平稳
性，常常使用对数收益率：

$${r_t} = \ln {R_t} - \ln {R_{t - 1}}$$

其中， ${R_{t - 1}}$ 为上一期的隔夜拆放利率，通过计算，可以得到包含3059个上海银行隔夜同业拆放的收益率
的时间序列。SHIBOR隔夜拆放利率和对数收益率时序图如下：
...
## 载入数据
#上海银行同业拆借率
data=read.csv("E:\\github_repo\\shibor-msgarch\\Shibor_data.csv",header=T)
shibor<-data[,2]
par(mfrow=c(1,2))
...
plot(shibor.date[-1],shibor.rt,type="l",xlab="日期",ylab="对数收益率",cex.main=0.95,las=1,main="图1 SHIBOR隔夜拆放利率和对数收益率时序图")
#显然具有波动应聚集的特征
```
由图1可以直观地看出，SHIBOR隔夜拆借利率在2006-2010年波动幅度较平稳，在2010-2016年
波动幅度较大，在2016-2018年波动趋于平稳。对数收益率序列的方差随时间变化而变化，而
且有时变化得十分剧烈。同时，对数收益率的波动存在明显的波动集群效应，大波动后面接
着大波动，小波动后接着小波动，大小波动之间存在明显的差异性。
...
建立模型之前对基础数据进行正态性检验是十分必要的。首先可以通过计算对数收益率数据的
偏度和峰度、直方图和QQ图进行描述性分析。

对数收益率的偏度为0.8151，峰度为31.2635，而正态分布的偏度和峰度分别为0和3，故对数
收益率序列呈现一定程度的右偏性和显著的尖峰特征，不服从正态分布。
...
以上均是描述性分析得出的初步判断，为了更加科学地判断收益率序列的特征，采用JB（Jarq
ue-Bera）统计量对收益率数据进行正态性检验，统计量JB对应的p值小于显著性水平0.05，故
拒绝原假设，验证了之前收益率数据不服从正态分布的判断。
```{r}
#平稳性检验
...
在对时间序列进行分析时，必须要进行平稳性检验。通过使用适用于异方差场合的PP检验法(P
hillips-Person Test)对SHIBOR对数收益率序列进行检验发现，该序列是平稳的。
```{r, eval=F, echo=T}
#自相关性检验
par(mfrow=c(1,2)) 
...
Ljung-Box Q统计量对SHIBOR对数收益率序列进行自相关分析。检验结果显示，收益率序列具
有较强的自相关性。
```{r, eval=F, echo=T}
...
由SHIBOR的对数收益率的时序图可以看出，收益率数据可能存在异方差现象。两种常用的ARCH
检验统计方法是Portmanteau Q检验和LM检验。这里使用Portmanteau Q检验对收益率数据进行
分析。根据R软件分析结果可知 $p - value < 0.05$ ，故拒绝原数据，即认为对数收益率平方
序列自相关，对数收益率存在ARCH效应。
```{r, eval=F, echo=T}
#拟合GARCH
library(timeDate)
