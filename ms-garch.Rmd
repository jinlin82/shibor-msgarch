...
其中最优模型为三阶段的MS-GARCH模型，且分布形式分别为有偏广义误差分布、有偏广义t分布、
有偏广义误差分布。
...
型是最优模型，即Shibor收益率拟合MS-GARCH模型时的最佳区制数为3。
...
表\@ref(tab:test)列出了2019-1-2到2019-1-8日的Shibor预测状况，结果表明，在未来5天内，虽然条
件波动率逐渐下降，但VaR有较大幅度的变化，最小的VaR出现在2019-1-2，对应最大的条件波动率，最
大的VaR出现在2019-1-8，对应最小的条件波动率；在预测期同时存在区制2和区制3的相互转换，且区制
3保持正收益率，区制2保持负收益率,相应的对应Shibor利率，则表现为先经历4天的下降后，最后一天利
率开始回升。
