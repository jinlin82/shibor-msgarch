...
为 `r shibor.rt2[1,4]` ，而正态分布的偏度和峰度均为0，故对数收益率序列具有右偏性
和尖峰性，初步判断不服从正态分布。
...
```
## ARMA-GARCH模型构建
```{r stat2, eval=T, echo=F,message=F,warning=F}
#ARMA-GARCH
...
ARCH项和GARCH项的阶数均为1。在分布类型的选取方面，基于3.2.1节的分析可知，SHIBOR对数收益率
序列存在偏态和尖峰厚尾特征，因此分别构建基于正态分布、t分布、ged以及它们各自的偏态分布的
ARMA(1,2)-GARCH(1,1)。 从表\@ref(tab:stat2)的 结果可以看出基于t分布的ARMA-GARCH模型显著优于基于正态分布和广义误差分布
的模型，且基于偏态t分布的模型表现更佳。最终根据参数显著性和AIC、BIC准则确定以上6个模型中的
最优模型为ARMA(1,2)-GARCH(1,1)-sstd。
...
构建MSGARCH模型时，同样选用GARCH(1,1)结构；考虑到对数收益率序列的分布特征，分布类
型依次选取传统的正态分布、对厚尾特征刻画更具灵活性的广义t分布和广义误差分布，以及
它们的偏态分布，即“norm”、"std"、"ged"、“snorm”、"sstd"、"sged"共6种。另外设置区制K
为2、3、4共3种情况。最终共得到51个模型，模型优度如表\@ref(tab:stat3)所示：

```{r stat3,eval=T,echo=F,message=F,warning=F}
...
其中包含k个区制的MS-GARCH模型用“MS(K)-GARCH”表示，包含k种分布的混合分布GARCH模型
用“MIX(k)-GARCH”表示。根据AIC与BIC准则，综合选择的最优模型为MS(4)-GARCH模型，且各区制
的分布均为sged分布。该模型的相关参数如表\@ref(tab:stat4)所示：

```{r stat4,eval=T,echo=F,message=F,warning=F}
...
该模型中所有参数均通过显著性检验，极大似然值、AIC和BIC值分别为`r m41$loglik`、`r summary(m41)$AIC`、`r summary(m41)$BIC`。
其中，omega表示4个区制中MS(4)-GARCH-sged中的常数项；alpha表示模型的ARCH项系数，
其值越大，表明当期收益率对下一期波动率的影响程度越大；beta表示模型的GARCH项系数，
该值表示当期波动率对下一期波动率的影响；skewed和shape分别指sged的偏度参数和形状参数，
shape值为1表明分布为对称分布，大于1 表明为右偏分布，小于1表明为左偏分布。因此该最优模型
在四个区制中分别为对称分布、右偏分布、右偏分布、左偏分布。

分区制对各项系数进行比较可以发现，第一区制中GARCH项系数值为`r m41$par[3]`，远远大于ARCH项系数
`r m41$par[2]`，表明在第一状态中，当期对数收益率序列的波动率对下一期的波动率起决定性作用，
这也说明第一区制的波动率具有聚集效应，具有长期记忆性。第三区制的结果类似。但在第二区制和第
四区制中，情况恰好相反，ARCH项系数在下一期的波动率变化中起更大作用，说明当期的收益率对
下一期的波动率影响更大。其中第四区制，GARCH项系数为0，表明第四区制为ARCH-sged模型，下
一期波动率只受当期收益率影响。以上分析进一步说明，对数收益率的确存在区制的转换，不同区制
呈现不同特征。

```{r stat5,eval=T,echo=F,message=F,warning=F}
...
表 \@ref(tab:stat5) 展示了该模型的转移矩阵，其中转移矩阵中的每个元素表示从当前状态转移
到下一状态的概率。以第一行为例，由于最优模型有4
个区制，故由当前t时刻的第一状态转移到下一时刻共有4种情况，4个元素分别对应转移到4个区制
的概率，其和为1。 $P_{11}$ 值为`r m41$par[21]`，表示在t时刻所处状态为第一区制的情况下，下一
时刻仍为第一区制的概率，其他元素类似。矩阵显示对角线元素在各行中均为最大值，即两个时刻连续
保持同一状态的概率最大，对数收益率的波动更容易保持一个状态不变。同时，尽管转移到其他状
态的概率较小，但状态之间的转移的确存在。若P值为0，则表示不存在相应的状态转换，如 $P_{42}$ 
的值为0，即表示不存在由第4区制转移到第2区制的情况，但第2区制转移到第4区制的情况以`r P2`
的概率发生。
