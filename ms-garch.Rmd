...
MSGARCH模型。根据参数显著性检验、极大似然值、AIC、BIC以及DIC准则的方法，其中最优
模型为三阶段的MS-GARCH模型，且分布形式分别为有偏混合正态分布、有偏广义t分布、有
偏混合正态分布分布。
...
`r m3$par[11]`，表示模型的ARCH项系数，其值越大，表明几乎不存在异方差，拆放利率的
波动在很大程度可由过去的利率波动和方差确定，即当期收益率对下一期波动率的影响程度
越大；beta分别为`r m3$par[3]`、`r m3$par[8]`、`r m3$par[12]`表示模型的GARCH项系数，
均小于1，表明上海同业拆放利率隔夜品种价格序列具备有限方差且属于弱平稳的过程，即前
期波动对当期影响具有较长的记忆性，外部冲击对波动性的影响随时间逐渐递减。
...
，无条件波动率为`r sqrt(250)*sapply(ExtractStateFit(m3),UncVol)[1]`，呈低频波动；
当对数收益率处于第二区制GARCH-std时，无条件波动率`r sqrt(250)*sapply(ExtractStateFit(m3),UncVol)[2]`，
...
```
如图 \@ref(fig:state) 所示，
通过对不同时刻所处的不同区制的分析以及波动率的表现特征，可以将隔夜拆借利率分为2006年至2010年、2010年至2014年、2014年至2018年三个时间段。

在第一阶段即2006年至2010年间，shibor利率有较为显著的集群现象，波动较为剧烈。主要原因在于2006年9月7日，
中国人民银行为提高金融机构自主定价能力，开始建立报价制货币市场基准利率(Shibor)。
而2007年至2008年期间，上海刚刚建立同业拆放市场，利率的形成机制尚处于萌芽阶段，政府为
了稳定这一新兴市场，会采取较多的干预措施，来扼制市场机制的运作，这使得shibor利率
时大时小，波动较大。在2008年，由于全球次贷危机的爆发，企业贷款资金需求量
下降，商业银行头寸资金需求也随之减小，shibor利率在该段时间也发生了急速的下降，
波动率陡增。

在中间阶段即2010年至2015年，shibor利率波动剧烈，有着显著的波动性聚集现象。主要原因是因为自2010年央行不断发挥Shibor
在货币市场中的作用，推动Shibor产品的发展以来，同业拆借利率的市场影响力不断提高，货币市场交易量
也大幅度增加。之后，在2013年6月，由于政府对外汇资金以及非法债券交易的严厉管控，使得
银行间流动性受到挤压，加之季节性流动性的需求增加，央行当年并没有及时采取相应措施，
使得Shibor经历了一场剧烈的波动，导致隔夜Shibor飙升。于2013年6月20日，隔夜Shibor利
率创历史新高，为13.444%。

在第三阶段即2015-2018年，同业拆借市场趋于平稳。这是因为同业拆放市场的机
制逐渐成熟，日趋完善，利率波动趋于一种平稳的状态。波动率
与金融市场的稳定性具有密切关联，商业银行应该重视并且充分利用Shibor波动表现，并能
够准确预测，从而有效控制政策信息、外部市场环境等带来的非预期风险，减小极端风险事
件带来的不必要的损失。


### VaR 计算
```{r var-real, eval=T,include=F}
#求VaR
...
收益率残差序列，红色虚线、绿色实线、蓝色虚线分别表示三个模型预测的相应VaR值。对预测的VaR值进行回测检验，三种检验方式得到的P值如表 \@ref(tab:test) 所示：

```{r test,eval=T,echo=F,include=T}
library("GAS")
...
test.print=data.frame(UC.pval,CC.pval,DQ.pval)
colnames(test.print)=c("MS(1)-norm","MS(2)-sstd-ged","MS(3)-sged-std-sged")
rownames(test.print)=c("Kupiec检验","条件覆盖测试","动态分位数测试")
kable(test.print,caption = "VaR回测检验")
```
综合考虑三种检验方式后，仅有三阶段的模型其P值均大于0.1，不拒绝原假设，因此最优区制为三区制。
###预测
...
pre=data.frame(pred$state,vol.pre,VaR0,shibor.rt.pre,shibor.pre)                                
colnames(pre)=c("所处状态","条件波动率","VaR","shibor收益率","shibor")
rownames(pre)=c("2019-1-2","2019-1-3","2019-1-4","2019-1-7","2019-1-8")

kable(pre,caption = "未来五天shibor预测状况")
```
# 结论
