...
表\@ref(tab:model-es)中包含k个区制的MS-GARCH模型用“MS(K)-GARCH”表示，包含k种分
布的混合分布GARCH模型用“MIX(k)-GARCH”表示。前6个模型为单一区制的GARCH模型，假
设分布为单一分布，即 $K = 1$ ，显然假设分布为t分布的模型显著优于正态分布和广义误
差分布; $K = 2$ 的两区制马尔科夫转换模型共有36个，其余的216个模型为 $K = 3$ 的
MSGARCH模型。根据参数显著性检验、极大似然值以及AIC和BIC准则的方法，其中最优模型
为三阶段的MS-GARCH模型，且分布形式分别为sged、std、sged分布。最终选择3阶段的
GARCH_sstd、GARCH_sstd、GARCH_ged为最优模型。
