...
对三种模型预测的VaR值进行回测检验，三种检验方式得到的检验统计量对应的P值如表 \@ref(tab:test) 所示：
...
test.print=t(data.frame(UC.pval,CC.pval,DQ.pval))
...
从表 \@ref(tab:test) 可以看出，仅有MS(3)模型在三种检验方式中均不拒绝原假设，表明该种类模型是最优模型，即Shibor收益率拟合MS-GARCH模型时的最佳区制数为3。
