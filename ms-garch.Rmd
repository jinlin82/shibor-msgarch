...
征，可以将隔夜拆借利率分为2006年至2010年、2010年至2014年、2014年至2018年三个时间
段。
...
熟，日趋完善，利率波动趋于一种平稳的状态。波动率与金融市场的稳定性具有密切关联，
商业银行应该重视并且充分利用Shibor波动表现，并能够准确预测，从而有效控制政策信息、
外部市场环境等带来的非预期风险，减小极端风险事件带来的不必要的损失。
...
率残差序列，红色虚线、绿色实线、蓝色虚线分别表示三个模型预测的相应VaR值。对预测
的VaR值进行回测检验，三种检验方式得到的P值如表 \@ref(tab:test) 所示：
...
综合考虑三种检验方式后，仅有三阶段的模型其P值均大于0.1，不拒绝原假设，因此最优区
制为三区制。
