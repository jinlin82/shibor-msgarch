...
${h_{k,t}}$随时间变化，${\xi _k}$是包含$\omega$ ,$\alpha$ ,$\beta$的形状参数向量，
例如表现为拖尾且不对称。${y_t}$的条件方差假定服从GARCH类模型。因此，在${s_t} =
k$的条件下，${h_{k,t}}$为第 $t$ 期的方差函数，${h_{k,t - 1}}$为上一期的方差，
${\theta _k}$是每个区制独立的条件向量参数，即：

${h_{k,t}} = h({y_{t - 1}},{h_{k,t - 1}},{\theta _k}) \tag{6}$

上式中$h(.)$是$t - 1$期可测量的函数，定义了条件方差并且确保了其正向性。

定义GARCH模型条件异方差部分，除了设定模型形式，还有残差分布，常用的条件分布形式
有：正态分布、学生t分布和广义误差分布。
