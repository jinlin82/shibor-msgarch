...
通过以上分析，分不同区制对各项系数进行比较，各个区制中衰减系数alpha+beta表示过去的
波动对未来的影响程度，从结果中我们可以看出，模型处于第一状态的衰减系数依次大于第二
、第三状态，说明第一状态是以更为缓慢的衰减产生更持久的波动。此结果与状态转移概率
${p_{11}}$ 、${p_{22}}$ 、${p_{33}}$ 的值也是对应的。此外， 第一区制中GARCH项系数
值为0.6812，大于ARCH项系数0.3183，表明在第一状态中，当期对数收益率序列的波动率对下
一期的波动率起决定性作用，这也说明第一区制的波动率具有聚集效应，具有长期记忆性。
但在第二区制和第三区制，情况恰好相反，ARCH项系数在下一期的波动率变化中起更大作用，
ARCH项系数为0.9631和0.3911，远大于GARCH项系数0.0174和0.0002，说明当期的收益率对下一
期的波动率影响更大。另外，通过计算，可以得到各个取值的无条件方差，即当对数收益率处于
第一区制GARCH-sged时，无条件波动率为0.1663889，呈低频波动；当对数收益率处于第二区制
GARCH-sstd时，无条件波动率0.4838072，波动幅度有所增加；当对数收益率处于第三区制GARCH
-ged时，其无条件波动率达到最高，为12.3301966，呈高频波动。可见SHIBOR对数收益率的波动
率有逐渐增大的趋势。以上分析综合说明，对数收益率序列存在区制的转换，不同区制呈现出不同特征。
...
### 最优模型意义
作为货币市场的“晴雨表”，shibor能够有效反映宏观经济的调控作用。若市场层面上资金紧张，投资者对
资金的需求量大于资金的供给量，则shibor会持续上升，反之下降。杠杆效应分为正杠杆效应和负杠杆效
应，综合以上对最优模型系数的解释，非对称系数xi_1和xi_3均为正，表明这是一种正杠杆效应，即坏消
息使得shibor的上升幅度大于好消息使得shibor下降的幅度，这有可能是由于参与交易的主体对于该市场
信息不足，以及市场本身机制不完善所导致的。商业银行应该重视并且充分利用shibor波动对利好消息和
利坏消息的反应的不对称性，从而有效控制政策信息、外部市场环境等带来的非预期风险，减小极端风险
事件带来的不必要的损失。

2006年9月7日，为提高金融机构自主定价能力，中国人民银行开始建立报价制货币市场基准利率，
（Shibor)。自2010年，央行不断发挥Shibor在货币市场中的作用，推动Shibor产品的发展，使得市场影响
力不断提高，货币市场交易量大幅度增加。之后，在2013年6月，由于政府对外汇资金以及非法债券交易的
严厉管控，使得银行间流动性受到挤压，加之季节性流动性的需求增加，央行当年并没有及时采取及时措
施，使得Shibor经历了一场剧烈的波动，导致隔夜Shibor飙升，创历史新高。此时，也符合图1中的时序图
，从12年的轨迹可以看出，Shibor主要有上涨、下跌和震荡三种状态，分别对应最优模型中的三种不同状
态。
