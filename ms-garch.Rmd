...
knitr::kable(res,caption = "MS(3)-GARCH-sged-std-sged模型估计参数总览",longtable = TRUE, booktabs = TRUE, linesep  = "")
```

表\@ref(tab:model-best)给出了最优模型的拟合结果，所有参数均通过显著性检验，极大似然值、AIC
和BIC值分别
为`r m3$loglik`、`r summary(m3)$AIC`、`r summary(m3)$BIC`。其中，omega 表示3个
区制 GARCH_sged、GARCH_std、GARCH_sged 中的常数项，其值分别为`r m3$par[1]`、`r m3$par[6]`、`r m3$par[10]`；
alpha分别为`r m3$par[2]`、`r m3$par[7]`、`r m3$par[11]`，表示模型的ARCH项系数，其
值越大，表明当期收益率对下一期波动率的影响程度越大；beta分别为`r m3$par[3]`、`r m3$par[8]`、`r m3$par[12]`表示模型的GARCH项系数，均小于1，表明上海同业拆解利率隔夜品种价格序列
具备有限方差弱平稳的特征，即当期波动率对下一期波动率的影响。
...
表\@ref(tab:shibor-pmatrix)展示了该模型的转移矩阵，其中转移矩阵中的每个元素表示从当前状态转移到下一状态的
概率。以第一行为例，由于最优模型有3个区制，故由当前t时刻的第一状态转移到下一时刻
共有3种情况，3个元素分别对应转移到3个区制的概率，其和为1。 $p_{11}$ 的值为`r m3$par[15]`，表
示在t时刻所处状态为第一区制的情况下，下一时刻仍为第一区制的概率，其他元素类似。
矩阵显示对角线元素在各行中均为最大值，即两个时刻连续保持同一状态的概率最大，对数
收益率的波动更容易保持一个状态不变。同时，尽管转移到其他状态的概率较小，但状态之
间的转移的确存在。若p值为0，则表示不存在相应的状态转换，如 $p_{31}$ 的值为`r m3$par[19]`，即表示不
存在由第3区制转移到第1区制的情况，但第1区制转移到第3区制的情况以`r P1`的概率发生，
表明不同状态之间的转换不具有对称性。

除了转移矩阵，同时还可以得到对数收益率序列的波动分别处于3个状态的无条件概率，结
果显示没有明显的概率差异，其中处于第一状态的概率最大，为`r summary(m3)$stable.prob[1]`，处于第二状态的概
率最小，为`r summary(m3)$stable.prob[2]`。 说明SHIBOR的对数收益率更倾向于在不同状态之间转化，且状态的无
条件概率几乎相同。
