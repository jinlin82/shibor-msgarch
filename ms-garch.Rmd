...
以上分别是马尔科夫转换模型以及最常用的GARCH(1,1)模型的简单介绍，MSGARCH是GARCH模
型的推广,允许在不同状态之间相互转化。而且在每个状态下，参数的设定也不同，都能很
好地拟合波动率聚集的现象。

令 ${y_t}$ 是时刻 $t$ 的日利率，我们关注动态的波动率，因此假定 $E({y_t}) = 0$ ,并
且 ${y_t}$ 序列不相关，令 $\left\{ {{S_t}}\right\}$
是有限集上的一条各态遍历齐次马尔可夫链。
...
显然${p_{i,j}}$需满足以下两个条件:
 $0 < {p_{i,j}} < 1,\forall i,j \in\left\{ {1, \cdots ,K} \right\} \tag{6}$
 $\sum\limits_{j = 1}^K {{P_{i,j}} = 1,} \forall i \in \left\{ {1, \cdots ,K} \right\} \tag{7}$

结合马尔科夫链$\left\{ {{S_t}} \right\}$的GARCH即为MSGARCH模型,下面给出一般表达式：
 ${y_t} = {h_t}{u_t} \tag{8}$
 $h_t^2 = {\omega _{{s_t}}} + {\alpha _{{s_t}}}\varepsilon _{t - 1}^2 + {\beta _{{s_t}}}h_{t - 1}^2 \tag{9}$

其中, ${\omega _{{s_t}}} \ge 0$ , ${\alpha _{{s_t}}} \ge 0$ , ${\beta _{{s_t}}} \ge 0$ 。用 ${I_{t - 1}}$ 表示 $t-1$ 期之前的历史信息, ${I_{t - 1}} = \left\{ {{y_{t - i}},i > 0} \right\}$ ,则MSGARCH可以表示为
 ${y_t}\left| {\left( {{s_t} = k,{I_{t - 1}}} \right)} \right. \sim D\left( {0,{h_{k,t}},{\xi _k}} \right) \tag{10}$
 ...
 ${h_{k,t}} = h({y_{t - 1}},{h_{k,t - 1}},{\theta _k}) \tag{11}$
