...
大量实践证据表明，经济和金融变量的时间序列，其行为特征可能会随着时间的推移呈现出不同的模式。 因此在构建模型时综合考虑时间序列的多个模式会更具科学性。 这可以通过马尔可夫转换机制，组合两个或更多个模型，构造马尔科夫转换模型来实现，下面通过一个例子对此作简单介绍。
$${z_t} = \left\{ {\begin{array}{*{20}{c}}
{{\alpha _0} + \beta {z_{t - 1}} + {\varepsilon _t},              {s_t} = 0,}\\
{{\alpha _0} + {\alpha _1} + \beta {z_{t - 1}} + {\varepsilon _t},{s_t} = 1,}
\end{array}} \right. \tag{1}$$
式中\left| \beta  \right| < 1，{{\varepsilon _t}}独立同分布，满足零均值，同方差。{{s_t}}遵循具有以下转移矩阵的一阶马尔可夫链。
...
$${y_t} ={h_t}{u_t} \tag{3}$$
$$h_t^2 =\omega+\alpha\varepsilon_{t-1}^2+\beta h_{t - 1}^2 \tag{4}$$
${y_t}$是时刻$t$的收益率，${\mu _t}$满足独立同分布，其均值为0，方差为1。为了确保条件方差存在且有界，一般限制$\omega > 0$,$\alpha > 0$,$\beta > 0$，当假设$\mu_t>0$为常数，则$\alpha+\beta$测
量条件方差的冲击持续性，$\alpha+\beta$之和越趋近于1，波动过程越持久。
