...
^{(s_t)}$ , $\alpha ^{(s_t)}$ , $\beta ^{(s_t)}$ 分别表示在当前区制下的各参数的值。不同
区制的转移由一个 $k$ 状态的的马尔科夫随机过程控制，状态转移矩阵如下：
...
混合正态分布是由两个或两个以上正态分布的加权组合形成的分布，且任意两个
正态分布之间均是独立的，因此混合正态分布也可以理解为马尔科夫转换模型的
一个特例。由于不同的正态分布有着不同的均值和方差,所以混合正态分布通常
比正态分布能够更好地刻画尖峰性和厚尾性，因此本文亦将使用混合分布GARCH模
型对shibor收益率数据进行拟合。模型估计部分采用R软件中的MSGARCH包实现。
...
银行间同业拆放利率(Shanghai Interbank Offered Rate,SHIBOR)自2006年10月成立以来，
在近年尤受重视。目前公布的品种依据时间划分主要包括隔夜（O/N）、1周（1W）、2周（2W）
、1个月（1M）等品种。

本文所收集的样本范围是2006年10月8日至2018年12月29日的隔夜同业拆借利率，总计3060个
SHIBOR隔夜拆放率数据。数据全部来自于上海银行间同业拆放利率官网(http://www.shibor.org/)。
假设 ${R_t}$ 为第 $t$  期的隔夜拆放利率，在探讨金融时间序列变化时，为了消除数据的非平稳
性，常常使用对数收益率：

$${r_t} = \ln {R_t} - \ln {R_{t - 1}}$$

其中， ${R_{t - 1}}$ 为上一期的隔夜拆放利率，通过计算，可以得到包含3059个上海银行隔夜
同业拆放的收益率的时间序列。SHIBOR隔夜拆放利率和对数收益率时序图如下：
