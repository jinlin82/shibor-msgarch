...
权定价模型[@Black1973]。J.P.Morgan(1983)首次提出了利率敏感性缺口模型，它是度量利
率风险的一个传统模型。Bollerslev(1986)运用GARCH模型对短期利率波动进行估计得到了
...
利率市场对数收益率在险值进行计算[@房小定2013]。曾裕峰等(2015)构建了基于CAViaR模
型的Shibor风险测度VaR模型，研究结果发现,CAViaR模型的风险预测效果优于传统的GARCH
族模型[@曾裕峰2015]。项卫星,李宏瑾(2014)指出SHIBOR具备市场代表性、基准性和稳定性，
符合货币市场基准利率的基本特征[@项卫星2014]。严佳佳，郭春松等(2015)基于
...
Mixing-GARCH混合分布和Markov-switching-GARCH马尔科夫转换模型对SHIBOR的变化进行深
入探究，以更好地拟合同业拆借利率市场的真实情况，使得VaR值估计结果更加精确。

本文第二部分对所使用的统计理论和模型进行简单介绍，第三部分进行实证分析，具体选用
上海银行间同业拆借利率数据对同业拆借利率市场在险价值VaR进行度量。考虑到SHIBOR数
据具有尖峰厚尾特征，引入混合数分别为k=2、3、4的混合分布来代替普通正态分布和厚尾
分布，选择不同种类组合的混合分布GARCH族模型对利率样本序列进行均值方程和方差的拟
合，计算同业拆借利率的在险价值，并与普通正态分布和某一厚尾分布下的VaR值进行对比，
从而找出合适的条件异方差模型进行VaR的计算，并使用Kupiec失败率检验法检验模型的精
准度。确定最优模型后，根据模型特性分析上海银行间同业拆借利率的波动性特征，得出最终结论。

# 马尔科夫转换 GARCH 模型介绍

金融时间序列的分布较正态分布而言，尾巴拖得更长，中间峰顶更尖，即具有尖峰厚尾的特
征。通常我们假设GARCH(p,q)模型的残差服从正态分布，但金融序列的尖峰厚尾现象使得正
态分布的假定可能会造成模型设定误差。因此，为了更好的拟合尖峰厚尾现象，通常使用具
有尖峰后尾性特征的t分布、广义误差分布（GED）分布来拟合残差的分布。但在很多情况下，
t分布和GED分布对于收益率残差序列的特征的拟合不够充分。故本文首先引入Mixing-GARCH
混合分布模型和Markov-switching-GARCH马尔科夫转换模型，然后再设置模型的残差分布对
收益率序列进行拟合，下面对MSGARCH模型进行介绍。

## 马尔科夫转换模型介绍

绝大多数金融序列的行为特征呈现多种模式，但出于参数估计的可实现性和模型设计的简洁性，具体研究时只考虑单一模型。马尔科夫转换模型可以解决以上问题，
即通过马尔科夫转换机制组合两个或更多个模型。下面通过一个例子对此作简单介绍。
$${z_t} = \left\{ {\begin{array}{*{20}{c}}
{{\alpha _0} + \beta {z_{t - 1}} + {\varepsilon _t},              {s_t} = 0,}\\
{{\alpha _0} + {\alpha _1} + \beta {z_{t - 1}} + {\varepsilon _t},{s_t} = 1,}
\end{array}} \right. \tag{1}$$
式中 $\left| \beta  \right| < 1$ , ${{\varepsilon _t}}$ 是均值为0，方差为1的独立同分布随机变量序列。${{s_t}}$ 遵循具有以下转移矩阵的一阶马尔科夫链。
$$P = \left[ {\begin{array}{*{20}{c}}
{IP\left( {{s_t} = 0\left| {{s_{t - 1}} = 0} \right.} \right)}&{IP\left( {{s_t} = 1\left| {{s_{t - 1}} = 0} \right.} \right)}\\
{IP\left( {{s_t} = 0\left| {{s_{t - 1}} = 1} \right.} \right)}&{IP\left( {{s_t} = 1\left| {{s_{t - 1}} = 1} \right.} \right)}
\end{array}} \right] = \left[ {\begin{array}{*{20}{c}}
{{p_{00}}}&{{p_{01}}}\\
{{p_{10}}}&{{p_{11}}}
\end{array}} \right] \tag{2}$$
 ${p_{i,j}}(i,j = 0,1)$ 表示从 ${s_{t - 1}} = i$ 转移到 ${s_t} = j$ 的概率，并且 $0 < {p_{i,j}} < 1$ , ${p_{i0}} + {p_{i1}} = 1$ 。该转移矩阵控制着状态变量 ${s_t} = j$ 的变化过程。
当 ${s_t} = 0$ 时,(1)式是均值为 ${\alpha _0}/\left( {1 - \beta } \right)$ 的AR(1)过程，当 ${s_t} = 1$ 时，(1)式转化成均值为 ${\alpha _0} + {\alpha _1}/\left( {1 - \beta } \right)$ 的AR(1)过程。因此, ${z_t}$ 由两个不同分布决定，并且 ${s_t}$ 控制着两个分布的转换。如此,(1)式这样含有马尔科夫状态变量的模型就叫马尔科夫转换模型。

## GARCH(1,1)

在金融领域中，经常用GARCH模型来描述方差序列的波动，它可以有效拟合具有长期记忆性的异方差函数。下面给出经典的GARCH(1,1)模型：
$${y_t} = {h_t}{\varepsilon _t} \tag{3}$$
$$h_t^2 = \omega  + \alpha y_{t - 1}^2 + \beta h_{t - 1}^2 \tag{4}$$
模型中， ${y_t}$ 表示时刻 $t$ 的收益率, $h_t^2$ 表示条件方差， ${\varepsilon _t}$ 是均值为0，方差为1的独立同分布随机变量序列。为了确保无条件方差存在且有界，一般限制 $\omega > 0$ , $\alpha > 0$ , $\beta > 0$ ；同时限制条件 $\alpha  + \beta  < 1$ 可以确保GARCH模型的平稳性。其中(3)式为均值方程，用来描述收益率序列条件均值的变化过程，(4)式为方差方程，用来描述收益率序列条件方差的变化过程。

## MSGARCH


以上分别是马尔科夫转换模型以及最常用的GARCH(1,1)模型的简单介绍，MSGARCH是G
ARCH模型的推广,允许在不同状态之间相互转化。而且在每个状态下，参数的设定也
不同，都能很好地拟合波动率聚集的现象。

令${y_t}$是时刻$t$的日利率，我们关注动态的波动率，
因此假定$E({y_t}) = 0$，并且${y_t}$序列不相关，令$\left\{ {{S_t}} \right\}$是一
个遍历齐次马尔可夫链，${s_t}$根据未观测到的一阶遍历齐次马尔可夫链进化，

令${y_t}$是时刻$t$的日利率，我们关注动态的波动率，因此假定$E({y_t}) = 0$，并且${y_t}$序列不相关，令$\left\{ {{S_t}}\right\}$是有限集上的一条各态遍历齐次马尔可夫链。

转移矩阵为$P = \left[ {\begin{array}{*{20}{c}}
{{p_{1,1}}}& \cdots &{{p_{1,K}}}\\
 \vdots & \ddots & \vdots \\
{{p_{K,1}}}& \cdots &{{p_{K,K}}}
\end{array}} \right]$

${p_{i,j}}$表示第t-1期从状态i到第t期状态j的转移概率，即从${s_{t - 1}} = i$到${s_t} = j$的概率。

显然${p_{i,j}}$需满足以下两个条件：
$0 < {p_{i,j}} < 1,\forall i,j \in\left\{ {1, \cdots ,K} \right\}$,$\sum\limits_{j = 1}^K {{P_{i,j}} = 1,} \forall i \in \left\{ {1, \cdots ,K} \right\}$

结合马尔科夫链$\left\{ {{S_t}} \right\}$的GARCH即为MSGARCH模型，下面给出一般表达式
${y_t} = {h_t}{u_t} \tag{3}$
$h_t^2 = {\omega _{{s_t}}} + {\alpha _{{s_t}}}\varepsilon _{t - 1}^2 + {\beta _{{s_t}}}h_{t - 1}^2 \tag{4}$

其中，${\omega _{{s_t}}} \ge 0$,${\alpha _{{s_t}}} \ge 0$,${\beta _{{s_t}}} \ge 0$.用${I_{t - 1}}$表示t-1期之前的历史信息，${I_{t - 1}} = \left\{ {{y_{t - i}},i > 0} \right\}$,则MSGARCH可以表示为
${y_t}\left| {\left( {{s_t} = k,{I_{t - 1}}} \right)} \right. \sim D\left( {0,{h_{k,t}},{\xi _k}} \right) \tag{5}$

$D\left( {0,{h_{k,t}},{\xi _k}} \right)$表示条件分布，服从零均值，条件方差
${h_{k,t}}$随时间变化，${\xi _k}$是包含$\omega$ ,$\alpha$ ,$\beta$的形状参数向量，例如表现为拖尾且不对称。${y_t}$的条件方差假定服从GARCH类模型。因此，在${s_t} =k$的条件下，${h_{k,t}}$为第 $t$ 期的方差函数，${h_{k,t -1}}$为上一期的方差，${\theta _k}$是每个区制独立的条件向量参数，即：
${h_{k,t}} = h({y_{t - 1}},{h_{k,t - 1}},{\theta _k}) \tag{6}$
上式中$h(.)$是$t - 1$期可测量的函数，定义了条件方差并且确保了其正向性。

定义GARCH模型的条件异方差部分，除了设定模型形式，还有残差分布，常用的条件
分布形式有：正态分布、学生t分布和广义误差分布。

混合正态分布是由两个或两个以上正态分布的加权组合形成的分布，它们之间是独立
的，有着不同的均值和方差,通常比正态分布能够更好刻画尖峰性和厚尾性。而且在
本文所使用的MSGarch包中通过设置参数可以得到混合正态分布的具体形式。因此，
我们将混合正态分布作为MSGarch的一种特殊情况来分析。

## 以2区制为例
假设当${s_t} = 0$时，波动率是${h_{0,t}^2}$,
${s_t} = 1$时，波动率是${h_{1,t}^2}$，假定${{u_t}}$服从正态分布，那么MSGARCH的$h_t^2$可表示为以下形式：

$h_{0,t}^2 = {\omega _0} + {\alpha _0}\varepsilon _{t - 1}^2 + {\beta _0}E\left[ {h_{t - 1}^2\left| {{I_{t - 2}}} \right.} \right]$,当${s_t} = 0$

$h_{1t}^2 = {\omega _1} + {\alpha _1}\varepsilon _{t - 1}^2 + {\beta _1}E\left[ {h_{t - 1}^2\left| {{I_{t - 2}}} \right.} \right]$,当${s_t} = 1$

$E\left[ {h_{t - 1}^2\left| {{I_{t - 2}}} \right.} \right] = h_{0,t - 1}^2\Pr \left[ {{s_{t - 1}} = 0\left| {{I_{t - 2}}} \right.} \right] + h_{1,t - 1}^2\Pr \left[ {{s_{t - 1}} = 1\left| {{I_{t - 2}}} \right.} \right]$.

以上公式表明，$h_t^2$仅依赖于第t期的${s_t}$。假定状态变量${s_t}$服从马尔科夫链，引入GARCH模型，即为：

$h_t^2 = {\omega _{{s_t}}} + {\alpha _{{s_t}}}\varepsilon _{t - 1}^2 + {\beta _{{s_t}}}h_{t - 1}^2$

## 模型估计
目前，MSGARCH和Mixture of GARCH模型的估计方法有两种，一种是极大似然估计法，另一
种是基于贝叶斯方法的MCMC法。

## MS-GARCH模型选择依据
普通GARCH模型只能刻画数据在一种状态时的方差的性质，但对于金融数据而言，外部冲击
往往会使数据的波动状况发生改变。比如央行近年频繁采取“降准”的货币政策，这会使银行
间短期资金供给增加，同业拆借利率降低，波动性也逐渐平缓。此时如果继续采用普通
...
往具有周期性，过去某一时期的波动性状况在未来也可能会复现，针对该种情况，就需要一
个状态转换的MS-GARCH模型来进行描述。
