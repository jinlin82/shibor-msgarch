...
[@eq:ms-model3]式中 $\mu ^{(s_t)}$ 表示 $y_t$ 在状态 $s_t$ 下的均值方程
...
\end{array}} \right.$$[@eq:ms-model5]

[@eq:ms-model5]式中 ${e_t}$ 是均值为 $0$ ，方差相等的独立同分布随机变量序列。
...
[@eq:ms-model6]式中 ${p_{i,j}}$ 表示从状态 $j$ 转移到状态 $i$ 的概率。
...
成立以来尤受重视。目前公布的品种依据时间划分主要包括隔夜（O/N）、 $1$ 周（ $1$ W）、
 $2$ 周（ $2$ W）、 $1$ 个月（ $1$ M）等品种。
