...
另外，本文存在以下几方面可继续研究：(1)文中所构建模型均设定条件方程为基本的GARCH(1,1)，
并未考虑收益率序列可能存在的杠杆效应，故在后续研究中，可以构建EGARCH模型和TGARCH模型对
条件波动率的非对称性进行解释；(2)在模型参数估计方面，本文使用了极大似然法，后续学者也
可以使用基于贝叶斯的MCMC算法进行参数估计；(3)本文在描述序列阶段性波动特征时引入的是
MSGARCH模型，MIXGARCH模型同样能够刻画金融时间序列的非线性特征，且具有良好的拟合效果。
