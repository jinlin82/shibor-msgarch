...
```{r fig-ts,eval=T,echo=FALSE,fig.cap = "SHIBOR隔夜拆放利率和对数收益率时序图", dev='png'}
par(mfrow=c(2,1))
par(mar=c(2,4,1,2))
...
由图 \@ref(fig:fig-ts)  可以看出，SHIBOR隔夜拆借利率在2006-2010年波动幅度较平稳，在2010-2016年
...
```{r stat, eval=T,echo=F,message=FALSE,warning=FALSE,comment=""}
...
从表 \@ref(tab:stat) 看出，对数收益率的偏度为`r shibor.rt2[1,3] `，峰度
为 `r shibor.rt2[1,4]` ，而正态分布的偏度和峰度均为0，故对数收益率序列具有右偏性
和尖峰性，初步判断不服从正态分布。

```{r,eval=T,echo=F,fig.align="center",fig.cap="SHIBOR收益率频率分布直方图和正态Q-Q图", dev='png'}
par(mfrow=c(1,2))
#绘制直方图
...
图2的直方图和QQ图也显示对数收益率序列不服从正态分布。直方图中，对数收益率序列的尾
...
```{r, eval=T, echo=F,fig.align="center",fig.cap="SHIBOR收益率自相关图和偏自相关图", dev='png'}
#自相关性检验
par(mfrow=c(1,2)) 
...
```{r, eval=T, echo=F,fig.align="center",fig.cap="SHIBOR收益率残差序列图", dev='png'}
plot(shibor.date[-1],res.ts,type="l",xlab="日期",ylab="残差",cex.main=0.95,las=1)
```
图3中的自相关系数和偏自相关系数均显示出拖尾的性质，考虑到模型的简洁性，本文将ARMA模型参数p,q
...
观察图4中的残差序列可以发现，虽然其波动是平稳的，但在2006-2008， 
