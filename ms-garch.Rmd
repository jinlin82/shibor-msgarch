...
m2<-garchFit(formula = ~arma(1,2)+garch(1,1),data =shibor.rt,trace = F,cond.dist = "std")
summary(m2)#
m2.res=c("m2",m2@fit$ics[-c(3,4)])
##拟合基于ged分布的arma(1,2)GARCH(1,1)模型
m3<-garchFit(formula = ~arma(1,2)+garch(1,1),data =shibor.rt,trace = F,cond.dist = "ged")
summary(m3)#所估参数的残差分析出现空值，舍去
m3.res=c("m3",m3@fit$ics[-c(3,4)])
##拟合拟合基于snorm分布的arma(1,2)GARCH(1,1)模型
m4<-garchFit(formula = ~arma(1,2)+garch(1,1),data =shibor.rt,trace = F,cond.dist = "snorm")
summary(m4)#
m4.res=c("m4",m4@fit$ics[-c(3,4)])
##拟合基于sstd分布的arma(1,2)GARCH(1,1)模型
m5<-garchFit(formula = ~arma(1,2)+garch(1,1),data =shibor.rt,trace = F,cond.dist = "sstd")
summary(m5)
...
res2=data.frame(m1.res,m2.res,m3.res,m4.res,m5.res,m6.res)#
res2#m5基于sstd分布的arma(1,2)GARCH(1,1)模型最优
```
...
ARMA(1,2)-GARCH(1,1)。结果显示基于t分布的ARMA-GARCH模型显著优于基于正态分布和广义误差分布
的模型，且基于偏态t分布的模型表现更佳。最终根据参数显著性和AIC、BIC准则确定以上6个模型中的
最优模型为ARMA(1,2)-GARCH(1,1)-st.
