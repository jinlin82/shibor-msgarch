...
第一状态中，当期对数收益率序列的波动率对下一期的波动率起决定性作用。外部冲击影响
较大。但在第二区制和第三区制，情况恰好相反，ARCH项系数在下一期的波动率变化中起更
大作用，ARCH项系数为`r m3$par[7]`和`r m3$par[11]`，远大于GARCH项系数`r m3$par[8]`
和`r m3$par[12]`，说明当期的收益率主要依赖于前一期的方差。
...
除了转移矩阵，同时还可以得到对数收益率序列的波动分别处于3个状态的条件概率，结
...
收益率震荡下行。央行波动率与金融市场的稳定性具有密切关联，商业银行应该重视并且充
分利用Shibor波动表现，并能够准确预测，从而有效控制政策信息、外部市场环境等带来的
非预期风险，减小极端风险事件带来的不必要的损失。
...
为13.4556，对应的p值为0.0618，在显著性水平为0.1时，有重复的理由可以拒绝原假设，
...
表 \@ref(tab:pre) 列出了2019-1-2到2019-1-8日的Shibor预测状况，结果表明，在未来5天内，虽然条
件波动率逐渐下降，但VaR有较大幅度的变化，最小的VaR出现在2019-1-2，对应最大的条件波动率，最
大的VaR出现在2019-1-8，对应最小的条件波动率；在预测期同时存在区制2和区制3的相互转换，区制3
对应有偏广义误差分布，区制2对应广义t分布；表最后一列是真实的消除自相关后的收益率。
