...
马尔科夫转换模型能够将金融时间序列划分为若干个区间，分别研究不同区间下的数据特征，可以更准确地描述金融市场真实情况。一般化的马尔科夫转换模型可以表示如下：

$${Y_t} = \beta _0^{{s_k}} + \sum\limits_{i = 1}^{{n_1}} {\beta _i^{{s_k}}{X_i}}  + \sum\limits_{i = 1}^{{n_2}} {\beta _i^{{s_k}}{Y_{t - j}}}  + {\varepsilon ^{{s_k}}},\;\;{\varepsilon ^{{s_k}}} \sim NID\left( {0,{\sigma ^{{s_k}}}} \right)\tag{1}$$

式中 $k$ 表示区制的个数，$n_1$ , $n_2$ 分别表示外生变量和自回归的阶数, $s_k$ 表示不同区制的状态。不同区制的转移遵循具有以下转移矩阵的马尔科夫随机过程：

$${P_{ij}} = \Pr \left( {{S_{t + 1}} = i\left| {{S_t} = j} \right.} \right)且\sum\limits_{i = 1}^k {{P_{ij}}}  = 1\tag{2}$$

其中 ${P_{i,j}}$ 表示从状态 $j$ 转移到状态 $i$ 的概率。

## GARCH模型介绍

金融时间序列往往具有显著的波动性聚集特征，如果继续使用传统的线性计量模型会使结果失真，为此Bollerslev在Engle的相关研究上提出了GARCH模型以解决类似问题，该模型结构如下：
$$\left\{ {\begin{array}{*{20}{c}}
{{y_t} = f\left( {t,{y_{t - 1}},{y_{t - 2}}, \cdots } \right) + {\varepsilon _t}}\\
{{\varepsilon _t} = \sqrt {{h_t}} {e_t}}\\
{{h_t} = \omega  + \sum\limits_{i = 1}^p {{\alpha _i}{h_{t - i}}}  + \sum\limits_{j = 1}^q {{\beta _j}{\varepsilon _{t - j}}} }
\end{array}} \right.\tag{3}$$

其中， ${y_t}$ 表示时刻 $t$ 的收益率, $h_t$ 表示条件方差， ${e_t}$ 是均值为0，方差相等的独立同分布随机变量序列。第一个方程为均值方程，用来描述收益率序列条件均值的变化过程，第三个方程为方差方程，用来描述收益率序列条件方差的变化过程。
