...
项系数值为`r m3$par[3]`，大于ARCH项系数`r m3$par[2]`，表明在第一状态中，当期对数
收益率序列的波动率对下一期的波动率起决定性
...
第二区制GARCH-std时，无条件波动率`r sqrt(306)*sapply(ExtractStateFit(m3),UncVol)[2]`，
波动幅度有所增加；当对数收益率处于第三区制GARCH-sged时，其无条件波动率达到最高，
为`r sqrt(306) * sapply(ExtractStateFit(m3), UncVol)[3]`，呈高频波动，
可见SHIBOR对数收益率的波动率有逐渐增大的趋势。以上分析综合说明，对数收益率序列存在
区制的转换，不同区制呈现出不同特征。
