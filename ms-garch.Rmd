...
kable(res.table, row.names =F, align = "c", caption="模型估计结果比较表",
      longtable = TRUE, booktabs = TRUE, linesep  = "")
```
...
为6377.884、-12715.768、-12595.251。其中，alpha0_1,alpha0_2,alpha0_3 分别表示3个
区制 GARCH_sstd、GARCH_sstd、GARCH_ged 中的常数项，其值均为0；
alpha1_1,alpha1_2,alpha1_3,分别表示模型的ARCH项系数，其值越大，表明当期收益率对
下一期波动率的影响程度越大；beta_1, beta_2、beta_3分别表示模型的GARCH项系数，该
值表示当期波动率对下一期波动率的影响；nu和xi均应大于0，分别指sged的尾部参数和偏
度参数，,xi_1的值为1.5997，大于1，表示第一取值的残差分布为右偏，xi_2的值为0.8344，
小于1，表示第二取值残差分布呈左偏。

分区制对各项系数进行比较可以发现，第一取值中GARCH项系数值为0.6472，大于ARCH项系
数0.35，表明在第一状态中，当期对数收益率序列的波动率对下一期的波动率起决定性作用，
这也说明第一区制的波动率具有聚集效应，具有长期记忆性。第三区制的结果类似且其长期
记忆性比第一取值更显著，GARCH项系数为0.9877。但在第二区制，情况恰好相反，ARCH项
系数在下一期的波动率变化中起更大作用，ARCH项系数为0.9364，远大于GARCH项系数
0.0609，说明当期的收益率对下一期的波动率影响更大。另外，通过计算，可以得到各个取
值的无条件方差，即当对数收益率处于第一取值GARCH-sstd时，无条件波动率为0.1663889，
呈低频波动；当对数收益率处于第二取值GARCH-sstd时，无条件波动率0.4838072，波动幅
度有所增加；当对数收益率处于第三取值GARCH-ged时，其无条件波动率达到最高，为
12.3301966，呈高频波动。可见SHIBOR对数收益率的波动率有逐渐增大的趋势。以上分析综
合说明，对数收益率序列存在区制的转换，不同区制呈现出不同特征。
...
共有3种情况，3个元素分别对应转移到3个区制的概率，其和为1。p_1_1，值为0.9141，表
...
存在由第3区制转移到第1区制的情况，但第1区制转移到第3区制的情况以0.0859的概率发生，
表明不同状态之间的转换不具有对称性。

除了转移矩阵，同时还可以得到对数收益率序列的波动分别处于3个状态的无条件概率，结
果显示没有明显的概率差异，其中处于第一状态的概率最大，为0.3425，处于第二状态的概
率最小，为0.3176. 说明SHIBOR的对数收益率更倾向于在不同状态之间转化，且在状态的无
条件概率几乎相同。
