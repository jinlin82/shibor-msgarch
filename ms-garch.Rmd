...
$${Y_t} = \beta _0^{{s_t}} + \sum\limits_{i = 1}^{{n_1}} {\beta _i^{{s_t}}{X_i}}  + \sum\limits_{i = 1}^{{n_2}} {\beta _i^{{s_t}}{Y_{t - j}}}  + {\varepsilon ^{{s_t}}},\;\;{\varepsilon ^{{s_t}}} \sim NID\left( {0,{\sigma ^{{s_t}}} \right)\tag{1}$$

式中 ${y_t:t=1,2,…,T}$ 表示金融时间序列， $k$ 表示区制的个数，$n_1$ , $n_2$
...
$${y_t} = {\mu _{{s_t}}} + {\varepsilon _{{s_t}t}}\tag4$$

其中 $\mu _{{s_t}}$ 表示 $y_t$ 在状态 $s_t$ 下的均值方程

$${\mu _{{s_t}}} = {\phi _{{s_t}0}} + {\phi _{{s_t}1}}{y_{t - 1}} +  \cdots  + {\phi _{{s_t}q}}{y_{t - q}}\tag5$$
...
{{h_{{s_t}t}} = {\omega _{{s_t}}} + \sum\limits_{i = 1}^p {{\alpha _{{s_t}i}}{h_{{s_t}t - i}}}  + \sum\limits_{j = 1}^q {{\beta _{{s_t}j}}{\varepsilon _{{s_t}t - j}}} }
\end{array}} \right.\tag6$$

其中 ${e_t}$ 是均值为0，方差相等的独立同分布随机变量序列。$\omega _{{s_t}},\alpha _{{s_t}},\beta _{{s_t}}$ 分别表示在当前区制下的各参数的值。
不同区制的转移由一个k状态的的马尔科夫随机过程控制，状态转移矩阵如下：

$$P = \left( {\begin{array}{*{20}{c}}
{{p_{11}}}&{{p_{12}}}& \cdots &{{p_{1k}}}\\
{{p_{21}}}&{{p_{22}}}& \cdots &{p{}_{2k}}\\
