...
结果详见表\@ref(tab:model-es)。

```{r model-es, eval=T, results='markup', cache=T}
...
${s_t}$ 和转移矩阵 $P$ ，可以实现不同区制的转换，从而对不同波动率进行不同的刻画描述，
使得模型精度更高。

表\@ref(tab:model-es)中包含k个区制的MS-GARCH模型用“MS(K)-GARCH”表示，包含k种分布
...
型。其中最优模型为三阶段的MS-GARCH模型，且分布形式分别为sged、std、sged分布。


### 最优模型选择及其解释
#### 系数表及其解释

```{r model-best, eval=T,results='markup', cache=T}
...
`r m3$par[11]`，表示模型的ARCH项系数，其值越大，表明当期收益率对下一
期波动率的影响程度越大；beta分别为`r m3$par[3]`、`r m3$par[8]`、`r m3$par[12]`表
示模型的GARCH项系数，均小于1，表明上海同业拆解利率隔夜品种价格序列具备有限方差弱
平稳的特征，即当期波动率对下一期波动率的影响。
...
$_{11}$ 的值为`r m3$par [15]`，表示在t时刻所处状态为第一区制情况下，下一时刻仍为
第一区制的概率，其他元素类似。矩阵显示对角线元素在各行中均最大值，即两个时刻连续
保持同一状态的概率最大，对数收益率的波动更容易保持一个状态变。同时，尽管转移到其
他状态的概率较小，但状态之间的转移的确存在。若p值为0，则表示不存在相应的状态转，
如 $p_{31}$ 的值为`r m3$par[19]`，即表示不存在由第3区制转移到1区制的情况，但第1
区制转移到第3区制的情况以`r P1`的概率发生，表明不同状态之间的转换不具有对称性。

除了转移矩阵，同时还可以得到对数收益率序列的波动分别处于3个状态的无条件概率，结
果显示没有明显的概率差异，其中处于第一状态的概率最大，为`r summary(m3)$stable.prob[1]`，
处于第二状态的概率最小，为`r summary(m3)$stable.prob[2]`。 说明SHIBOR的对数收
益率更倾向于在不同状态之间转化，且状态的无条件概率几乎相同。

#### 平滑概率图及其解释
```{r smooth-probability,fig.cap="平滑概率图", dev="png"}
...
par(mfrow=c(3,1))
plot(zoo::zoo(smoothed.prob1, order.by = zoo::index(shibor.rt.r)), lty = 1, plot.type = "single", las = 1,main="第一区制" ,ylab = "", xlab = "日期", lwd = 1, cex.axis = 1.5, cex.lab = 1.5)
plot(zoo::zoo(smoothed.prob2, order.by = zoo::index(shibor.rt.r)), lty = 1, plot.type = "single",las = 1, main="第二区制",ylab = "", xlab = "日期", lwd = 1, cex.axis = 1.5, cex.lab = 1.5)
plot(zoo::zoo(smoothed.prob3, order.by = zoo::index(shibor.rt.r)), lty = 1, plot.type = "single", las = 1,main="第三区制",ylab = "", xlab = "日期", lwd = 1, cex.axis = 1.5, cex.lab = 1.5)
```
...
项系数值为`r m3$par[3]`，大于ARCH项系数`r 
m3$par[2]`，表明在第一状态中，当期对数收益率序列的波动率对下一期的波动率起决定性
作用，这也说明第一区制的波动率具有聚集效应，具有长期记忆性。但在第二区制和第三区制，
情况恰好相反，ARCH项系数在下一期的波动率变化中起更大作用，ARCH项系数为`r m3$par[7]`
和`r m3$par[11]`，远大于GARCH项系数`r m3$par[8]`和`r m3$par[12]`，说明当期
的收益率对下一期的波动率影响更大。另外，通过计算，可以得到各个取值的无条件方差，
即当对数收益率处于第一区制GARCH-sged时，无条件波动率为
`r sqrt(306)*sapply(ExtractStateFit(m3),UncVol)[1]`，呈低频波动；当对数收益率处于
第二区制GARCH-std时，无条件波动率`r sqrt(306) * 
sapply(ExtractStateFit(m3),UncVol)[2]`，波动幅度有所增加；当对数收益率处于第三区制
GARCH-sged时，其无条件波动率达到最高，为`r sqrt(306) * sapply(ExtractStateFit(m3), 
UncVol)[3]`，呈高频波动，可见SHIBOR对数收益率的波动率有逐渐增大的趋势。以上分析综合
说明，对数收益率序列存在区制的转换，不同区制呈现出不同特征。
...
练集，滚动预测2016年3月15日至2018年12月29日共700天的在险价值，VaR回测图如 图
\@ref(fig:fig-ts99) 所示：
